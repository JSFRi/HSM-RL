...
#load tier tables
tier_list=np.load('./Tier_list_RL_20000_fixgenreq_allinit123_temp46_tempdesc20.npy',allow_pickle=True)

tier3=pd.DataFrame(tier_list[999][2],columns=['No.','No.','weight','temp','time_untouched','req_time'])
tier2=pd.DataFrame(tier_list[999][1],columns=['No.','No.','weight','temp','time_untouched','req_time'])
tier1=pd.DataFrame(tier_list[999][0],columns=['No.','No.','weight','temp','time_untouched','req_time'])

tier3=tier3.loc[:, ~tier3.columns.duplicated()]
tier2=tier2.loc[:, ~tier2.columns.duplicated()]
tier1=tier1.loc[:, ~tier1.columns.duplicated()]

## Initail parameters
b1t1=7.33/0.2
a1t1=np.exp(b1t1*0.35)
b2t1=7.33/200000
a2t1=np.exp(b2t1*360000)
b3t1=7.33/1.8
a3t1=np.exp(b3t1*10)
...
b2t2=7.33/170000
a2t2=np.exp(b2t2*500000)
b3t2=7.33/2
a3t2=np.exp(b3t2*10)

b1t3=7.33/0.25
a1t3=np.exp(b1t3*0.65)
b2t3=7.33/250000
a2t3=np.exp(b2t3*600000)
b3t3=7.33/4
a3t3=np.exp(b3t3*10)
...
if os.path.exists('./Heatmap_RL_zeroinit123-20000+20000-temp46-fixgenreq-tempdesc20/')==False:
    os.mkdir('./Heatmap_RL_zeroinit123-20000+20000-temp46-fixgenreq-tempdesc20/')
...
for turn in range(1000):
    print('%dth turn'%turn)
    if turn%10==0:
        new=pd.read_csv('./New_files_20000/new_file_%d.csv'%(turn//10))
        env1.tier=pd.concat([env1.tier,new],ignore_index=True)
    t1=time.time()
    ## generate requests
    ...
    Request=pd.read_csv('./Requests_new_20000/req_%d.csv'%turn)
    ...
    heat1=list(env1.tier['temp'])+[0]*(34225-len(env1.tier['temp']))
    matrix_heat1=[]
    for i in range(len(heat1)//185):
        matrix_heat1.append(heat1[185*i:(185*i+185)])
    ...
    plt.savefig('./Heatmap_RL_zeroinit123-20000+20000-temp46-fixgenreq-tempdesc20/heatmap_%d.png'%turn,format='png',dpi=320)
    #plt.show()
    plt.close()
...
np.save('transfer_list_RL_20000+20000_fixgenreq_zeroinit123_temp46_tempdesc20.npy',np.array(transfer_list_RL))
np.save('response_list_RL_20000+20000_fixgenreq_zeroinit123_temp46_tempdesc20.npy',np.array(response_list))
np.save('s123_list_RL_20000+20000_fixgenreq_zeroinit123_temp46_tempdesc20.npy',np.array([s1t1_list,s2t1_list,s3t1_list,s1t2_list,s2t2_list,s3t2_list,s1t3_list,s2t3_list,s3t3_list]))
np.save('request_list_RL_20000+20000_fixgenreq_zeroinit123_temp46_tempdesc20.npy',np.array(request_list))
np.save('Tier_list_RL_20000+20000_fixgenreq_zeroinit123_temp46_tempdesc20.npy',np.array(tier_list))
np.save('p_list_RL_20000+20000_fixgenreq_zeroinit123_temp46_tempdesc20.npy',np.array(p_list))